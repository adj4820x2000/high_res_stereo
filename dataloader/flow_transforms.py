...
class Scale(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[2] = cv2.resize(inputs[2], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
        inputs[3] = cv2.resize(inputs[3], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
        inputs[4] = cv2.resize(inputs[4], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
        inputs[5] = cv2.resize(inputs[5], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
        inputs[6] = cv2.resize(inputs[6], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
        inputs[7] = cv2.resize(inputs[7], None, fx=ratio,fy=ratio,interpolation=cv2.INTER_NEAREST)
...
class RandomCrop(object):
    ...
    def __call__(self, inputs,target):
        ...
        inputs[4] = inputs[4][y1: y1 + th,x1: x1 + tw]
        inputs[5] = inputs[5][y1: y1 + th,x1: x1 + tw]
        inputs[6] = inputs[6][y1: y1 + th,x1: x1 + tw]
        inputs[7] = inputs[7][y1: y1 + th,x1: x1 + tw]
