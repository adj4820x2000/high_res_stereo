...
class unet(nn.Module):
    def __init__(self):
        ...
        # Encoder
        self.convbnrelu1_1 = conv2DBatchNormRelu(in_channels=24, k_size=3, n_filters=16,
...
class Segnet(nn.Module):
    def __init__(self):
        ...
        self.convbnrelu1 = conv2DBatchNormRelu(in_channels=4, k_size=7, n_filters=32,
                                                 padding=1, stride=2, bias=False)
        self.convbnrelu1_b = conv2DBatchNormRelu(in_channels=32, k_size=7, n_filters=32,
                                                 padding=1, stride=1, bias=False)
        self.convbnrelu2 = conv2DBatchNormRelu(in_channels=32, k_size=5, n_filters=64,
                                                 padding=1, stride=2, bias=False)
        self.convbnrelu2_b = conv2DBatchNormRelu(in_channels=64, k_size=5, n_filters=64,
                                                 padding=1, stride=1, bias=False)
        self.convbnrelu3 = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=128,
                                                 padding=1, stride=2, bias=False)
        self.convbnrelu3_b = conv2DBatchNormRelu(in_channels=128, k_size=3, n_filters=128,
                                                 padding=1, stride=1, bias=False)
        ...
        self.upconv2 = nn.Sequential(nn.Upsample(scale_factor=2),
                                    conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=32,
                                                 padding=1, stride=1, bias=False))
        self.iconv2 = conv2DBatchNormRelu(in_channels=64+num_channel, k_size=3, n_filters=32,
                                                 padding=1, stride=1, bias=False)

        self.upconv1 = nn.Sequential(nn.Upsample(scale_factor=2),
                                    conv2DBatchNormRelu(in_channels=32, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False))
        self.iconv1 = conv2DBatchNormRelu(in_channels=16+num_channel, k_size=3, n_filters=16,
                                                 padding=1, stride=1, bias=False)
        
        self.pred4 = nn.Sequential(conv2DBatchNormRelu(in_channels=128, k_size=3, n_filters=num_channel,
                                                 padding=0, stride=1, bias=False),
                                    nn.Sigmoid())
        self.pred3 = nn.Sequential(conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=num_channel,
                                                 padding=0, stride=1, bias=False),
                                    nn.Sigmoid())
        self.pred2 = nn.Sequential(conv2DBatchNormRelu(in_channels=32, k_size=3, n_filters=num_channel,
                                                 padding=0, stride=1, bias=False),
                                    nn.Sigmoid())
        self.pred1 = nn.Sequential(conv2DBatchNormRelu(in_channels=16, k_size=3, n_filters=num_channel,
                                                 padding=0, stride=1, bias=False),
                                    nn.Sigmoid())
    ...
    def forward(self, x):
        H = x.shape[2]
        W = x.shape[3]
        conv1 = self.convbnrelu1(x)
        conv1b = self.convbnrelu1_b(conv1)
        conv2 = self.convbnrelu2(conv1b)
        conv2b = self.convbnrelu2_b(conv2)
        conv3 = self.convbnrelu3(conv2b)
        conv3b = self.convbnrelu3_b(conv3)

        pred4  = self.pred4(conv3b)

        upconv3 = self.upconv3(conv3b)
        i3_in = torch.cat([upconv3, conv2b], 1)
        iconv3 = self.iconv3(i3_in)
        pred3  = self.pred3(iconv3)
        #pred3_up = F.upsample(pred3, (int(H/2), int(W/2)))

        upconv2 = self.upconv2(iconv3)
        upconv2 = F.upsample(upconv2, (conv1b.shape[2],conv1b.shape[3]))
        pred3_up = F.upsample(pred3, (conv1b.shape[2],conv1b.shape[3]))
        i2_in = torch.cat([upconv2, conv1b, pred3_up], 1)
        iconv2 = self.iconv2(i2_in)
        pred2  = self.pred2(iconv2)
        #pred2_up = F.upsample(pred2, (H, W))

        upconv1 = self.upconv1(iconv2)
        pred2_up = F.upsample(pred2, (upconv1.shape[2],upconv1.shape[3]))
        i1_in = torch.cat([upconv1, pred2_up], 1)
        iconv1 = self.iconv1(i1_in)
